...
/*   Updated: 2018/01/19 20:00:12 by paperrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

static void		process_input_mouse(t_app *app)
{
	...
	if (x != last_x)
		app->cam.rot.s[1] -= 0.05 * ((x < last_x) * 2 - 1);
	if (y != last_y)
		app->cam.rot.s[0] -= 0.05 * ((y < last_y) * 2 - 1);
	last_x = x;
	last_y = y;
}

static void		process_input_key(t_app *app)
{
	...
	if (a_pressed || d_pressed)
		camera_translate_x(&app->cam, (d_pressed - a_pressed) * move);
	if (r_pressed || f_pressed)
		camera_translate_y(&app->cam, (r_pressed - f_pressed) * move);
	if (w_pressed || s_pressed)
		camera_translate_z(&app->cam, (w_pressed - s_pressed) * move);
	...
	if (j_pressed || l_pressed)
		app->cam.rot.s[1] += (l_pressed - j_pressed) * rot;
	if (i_pressed || k_pressed)
		app->cam.rot.s[0] += (k_pressed - i_pressed) * rot;
	if (y_pressed || h_pressed)
		app->cam.rot.s[2] += (h_pressed - y_pressed) * rot;
}

void			process_input(t_app *app)
{
	process_input_mouse(app);
	process_input_key(app);
}
