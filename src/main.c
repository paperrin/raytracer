...
/*   Updated: 2018/04/10 21:46:27 by eabgrall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			main(int ac, char **av)
{
	...
	*obj = obj_sphere(vec3r(0, 0, 0), 3, 0);

/*	
	if (!(obj = (t_obj*)ft_vector_push_back(&app.scene.v_obj, NULL)))
		return (error_cl_code(CL_OUT_OF_HOST_MEMORY));
	*obj = obj_plane(vec3r(0, 0, 0), vec3r_norm(vec3r(0, 1, 0)), vec3r(1, 0, 0), 0);
	obj->material_id = 0;
*/
	if (!(obj = (t_obj*)ft_vector_push_back(&app.scene.v_obj, NULL)))
		return (error_cl_code(CL_OUT_OF_HOST_MEMORY));
	*obj = obj_cone(vec3r(0, 0, 0), vec3r_norm(vec3r(0, 1, 0)), vec3r_norm(vec3r(1, 0, 0)), M_PI / 10);
	obj->material_id = 1;
	...
	light->color = vec3f(0.2, 1, 1);
	light->intensity = 100;
	light->as.point.pos = vec3r(-0.2, -1.5, 0);

	if (!(light = (t_light*)ft_vector_push_back(&app.scene.v_light, NULL)))
		return (error_cl_code(CL_OUT_OF_HOST_MEMORY));
	light->type = light_type_point;
	light->color = vec3f(0.8, 1, 0.2);
	light->intensity = 100;
	light->as.point.pos = vec3r(0.2, 1.5, 0);
