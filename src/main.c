...
/*   Updated: 2018/03/17 21:46:37 by tlernoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			app_create(t_app *app)
{
	...
	if (!(opencl_create(&app->ocl, 1)) && !error_string("opencl"))
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_ray_gen_primary_create(app) && !error_string("primary create")) 
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_ray_trace_create(app) && !error_string("trace create"))
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_prefix_sum_create(app) && !error_string("sum create"))
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_ray_sort_create(app) && !error_string("sort create"))
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_clear_create(app))
		app_destroy(app, EXIT_FAILURE);
	if (!kernel_ray_shade_create(app) && !error_string("shade create"))
		app_destroy(app, EXIT_FAILURE);
...
void		render(void *user_ptr, double elapsed)
{
	...
	if (!kernel_ray_gen_primary_launch(app) && !error_string("primary launch"))
		app_destroy(app, EXIT_FAILURE);
	...
	while (++depth <= 0)
	{
		if (!kernel_ray_trace_launch(app) && !error_string("trace launch"))
			app_destroy(app, EXIT_FAILURE);
		if (app->should_sort_rays && !kernel_prefix_sum_launch(app) && !error_string("sum launch"))
			app_destroy(app, EXIT_FAILURE);
		if (app->should_sort_rays && !kernel_ray_sort_launch(app) && !error_string("sort launch"))
			app_destroy(app, EXIT_FAILURE);
		rays_traced += app->n_rays;
		if (!kernel_ray_shade_launch(app) && !error_string("shade launch"))
			app_destroy(app, EXIT_FAILURE);
