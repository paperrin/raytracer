...
/*   Updated: 2017/12/18 21:09:08 by paperrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			render(t_app *app)
{
	...
	*obj = obj_sphere(vec3r(0, 0, 0), 1, 0);
	...
	opencl_kernel_set_ocl_nb_args(&app->kernel_ray_trace, &ocl, 4);
	...
	if (err != CL_SUCCESS)
		return (catch_error(err));
	clSetKernelArg(app->kernel_ray_trace.kernel, 0, sizeof(cl_mem), (void*)&i_ray_states);
